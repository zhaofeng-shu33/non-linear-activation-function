...
Now we consider $\xi(z) = a_0 + a_1 z + a_2 z^2 + \dots + a_{n-1} z^{n-1} + a_m z^m $, there will be an $(m+1) \times (m+1) $ matrix $M$, a vector  $q = [a_0, a_1, \dots, a_m]^T$ and  the quadratic form as $ q^T M q $. The element $M_{ij}$ is the coefficient of $a_ia_j$. $M_{00} = (1-r) n $, which is calculated in previous. Since $M$ is symmetric we only consider $i\leq j$.
...
Then we have $q^T N q = 1$ where $N_{ij} = \frac{r^{(i+j) / 2}}{n^{(i+j)/2 -1}} (i+j -1)!!$ when $i+j$ is even and $N_{ij} = 0$ when $i+j$ is odd. The special case is $N_{00} = n$. Now if we rescale the parameter: $b_0 = \sqrt{n} a_0, b_1 = a_1, b_2 = \frac{1}{\sqrt{n}} a_2$ and $b_m = \frac{1}{n^{(m-1)/2}} a_m$. Let $p = [b_0, b_1, \dots, b_m]$. Then $p^T \widetilde{N} p =1$ where $\widetilde{N}_{ij} = r^{(i+j) / 2} (i+j -1)!!$. Also, the object function $q^T M q$ is simplified as $q^T \widetilde{M} q$ where $\widetilde{M}_{ij} =  -(i-1)(j-1)(1-r)r^{(i+j)/ 2 } (i+j-3)!! $. This transformation makes the matrix irrelevant with $n$.
